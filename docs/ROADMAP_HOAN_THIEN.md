...
| ✅ 14 | P3.3 Backup & restore — XONG S86: `IBackupService`+`BackupService` zip db/recipes/users/points/config (`am-backup-*` tay chọn thư mục, `am-auto-*` mỗi ngày 1 bản giữ `Backup:KeepCount`=7, `am-prerestore-*` TỰ tạo trước mọi lần phục hồi); restore giải nén đè có chặn path-traversal + yêu cầu khởi động lại; Settings thẻ "Sao lưu & phục hồi" hết placeholder (Admin, confirm 2 bước đỏ); 3 test | Bảo mật/Ops | 1 | — |
| ✅ 15 | P4.1 Single-step — XONG S88: engine `SingleStep`/`IsWaitingStep`/`StepOnce` (gate tự cài sau MỖI nhóm order, bất biến 5 — station không biết; StepOnce KHÔNG mở gate của RequestPause thật — pause thật vẫn phải Resume có resume-check); Shell: toggle "Từng bước" action bar (Engineer+, chấm ● khi bật) + nút "Bước tiếp ▶" trên banner; 3 test | Chức năng | 1 | — |
| ✅ 16 | P4.2 Sequence per-recipe — XONG S88: `RecipeBase.SequenceFile` (khai tường minh) → convention `recipes/{Name}.sequence.json` → file mặc định; `SequenceSource` nghe RecipeChanged → invalidate + **validate sớm** (sequence recipe mới hỏng → alarm 60005 NGAY lúc đổi); 4 test | Chức năng | 0.5 | — |
| ✅ 17 | P4.3 Settings hoàn thiện — XONG S88: thẻ "Phần cứng" (bảng thiết bị: driver Sim/thật + trạng thái poll 1s + reconnect TỪNG thiết bị Engineer+ có audit) + thẻ "Host" (endpoint từ config read-only + trạng thái sống) + nút vào/thoát kiosk (Engineer+, `IKioskService` — Ctrl+Shift+F11 thành dự phòng). **Settings hết placeholder** | UI | 1 | — |
| ✅ 18 | P4.4 Production/SPC/ca — XONG S88: `ProductionOptions` (config `AutoMachine:Production`: ShiftStartHour/LengthHours + YieldWarn/Alarm) — Dashboard + Production cùng MỘT định nghĩa "ca hiện tại" (hết cửa sổ trượt 8h cứng); KPI yield **màu-khi-có-nghĩa** theo ngưỡng (vàng<Warn, đỏ<Alarm — hoãn từ ADR 0010 nay xong); export CSV record theo cửa sổ; trend theo giờ (yield bar + X̄ cycle + n — SPC đơn giản); 10 test | Chức năng | 1 | — |
