...
## 🗓️ Cập nhật lần cuối
**Ngày:** 2026-07-18
**Session:** #97 — **Thiết kế lại khu điều khiển trục theo mockup đã chốt (tham khảo 3 màn manual máy công nghiệp) — tách trạng thái ↔ chuyển động, bỏ panel Phản hồi**. Chủ dự án chê bố cục gộp S96, đưa mockup HTML tương tác + 3 ảnh màn tham khảo → chuyển thành WPF: **(1) Bảng "Trạng thái trục"** (trái) chỉ còn trạng thái + an toàn: tên trục · vị trí mono 18px · **ma trận 7 LED CÓ TIÊU ĐỀ CỘT** (ALM/+LIM/−LIM/ORG/E-STOP/IN-POS/SVO — thay dải chấm không nhãn S96 phải tra chú thích) · 3 nút SVO/Home/Clear; **bấm vùng trạng thái của hàng = chọn trục** (nút nền transparent + nội dung IsHitTestVisible=False), hàng đang chọn nền xanh; per-row ô đích/tốc/"Tới" BỎ — hết cảnh ô Tới rải từng hàng còn jog điều khiển trục khác. **(2) DOCK JOG** (phải) cho TRỤC ĐANG CHỌN: tên to + vị trí live · **banner interlock ĐỎ nêu lý do** (trục alarm → "Clear trước"; servo OFF → "bật servo trước") + jog/move tự disable · segmented **Jog liên tục** (giữ-để-chạy deadman P1.5 cũ) / **Jog bước** (chip 0.001/0.01/0.1/**1**/tùy ý — gộp phần Bước inching cũ, thêm StepOne) · JOG− · STOP · JOG+ (STOP không bao giờ khoá) · slider tốc độ theo trục · ô **Đi tới vị trí tuyệt đối**. **(3) Panel "Phản hồi" 4 đại lượng BỎ HẲN** theo yêu cầu — vị trí hiện tại trên dock đủ. VM mới: IsStepMode, StepOne, JogInterlockText/IsJogInterlocked (refresh mỗi tick poll + khi đổi trục). i18n +12 key ×3, dọn 11 key mồ côi (PerAxis/Relative/Absolute/Inching*/Feedback/MoveTo/Speed/ServoAlarmClear). **Smoke UIA**: bảng + 7/7 header LED + dock hiện · panel Phản hồi = 0 · **chọn AX_1 → banner "Servo đang OFF — bật servo..." → bấm SVO ON → banner tự tắt** · đủ Liên tục/Bước/Tốc độ/GoAbs · bảng điểm + chạy lặp vẫn cùng màn không cuộn · app sống. Build 0 warning; 340 test giữ nguyên (VM thêm thuần UI-state). **Còn hàng đợi**: P5 máy thật.
**Commit:** `(điền sau)`  ·  (S96: `598ce82` · S95: `ea31a51` · S94: `bbdce33`)

---

## 🗓️ Session #96
**Session:** #96 — **Pane Trục & Điểm vừa MỘT màn — bỏ trùng lặp, hết cuộn dọc (yêu cầu chủ dự án)**. Rà toàn pane tìm nội dung trùng: **(1) Card "Bảng đèn 8 tín hiệu" BỎ HẲN** — trùng lớn nhất: Alarm đã hiện bằng tên trục đỏ + Servo đã có nút ON/OFF ngay hàng trục (hiển thị 2 nơi); 6 tín hiệu còn lại (+Limit/−Limit/Origin/E-Stop/Zero/In-Pos) thành **dải LED 11px trong TỪNG hàng trục** (tooltip song ngữ từng đèn + 1 dòng chú thích cuối card) — tiết kiệm ~240px dọc; **(2) Card "Phản hồi servo" riêng → gộp vào card Jog** dạng lưới 2×2 compact (đủ 4 đại lượng Follow err/Velocity/Torque/Motor load) — tiết kiệm ~80px; **(3) Khung pane đổi ScrollViewer-cả-trang → Grid 2 hàng cố định**: hàng trên = điều khiển trục (trái) + jog/phản hồi (phải), hàng dưới = bảng điểm — **bảng điểm cuộn NỘI BỘ** (MaxHeight 140, ~4 hàng) khi máy nhiều điểm, trang ngoài KHÔNG BAO GIỜ cuộn; hàng trục nén padding 8→6, hàng điểm 40→33px; hint 2-chạm tự ẩn khi đã có thanh chọn (thanh chọn tự giải thích); JogHint chuyển thành tooltip trên jog pad; key i18n mồ côi (Axis.SignalTable/Axis.Col) xoá 3 ngữ. **Smoke UIA**: card bảng đèn cũ = 0, legend LED + feedback gộp hiện, **khối Chạy lặp + điểm Home/PickUp on-screen NGAY không cần cuộn** (trước S96 phải SetScrollPercent 100% mới thấy), jog + phanh Z + bảng điểm cùng màn, chọn PickUp → thanh chọn + 3 nút Tới/Teach/Lưu on-screen bấm được; app sống. Build 0 warning; XAML-only — 340 test giữ nguyên. **Bẫy môi trường ghi thêm**: app instance cũ còn chạy giữ khoá dll trong bin → build "succeeded" nhưng dll KHÔNG refresh (MSB3026 chỉ hiện khi build lại) — taskkill app TRƯỚC khi build. **Còn hàng đợi**: P5 máy thật.
**Commit:** `598ce82`  ·  (S95: `ea31a51` · S94: `bbdce33` · S93: `4073c4c`)
